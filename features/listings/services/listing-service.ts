...
import { createListingCode } from "@/features/listings/factories/listing-code-factory";
import { categoryFollowRepository } from "@/features/listings/repositories/category-follow-repository";
import { listingRepository, type ListingCreateInput, type ListingUpdateInput } from "@/features/listings/repositories/listing-repository";
import {
  canManageListing,
  hasListingPhoto,
  ...
  type ListingPayload
} from "@/features/listings/specifications/listing-specification";
import { ListingWriteWorkflow } from "@/features/listings/templates/listing-write-workflow";
...
class CreateListingWorkflow extends ListingWriteWorkflow<CreateInput, Awaited<ReturnType<typeof listingRepository.create>>> {
  protected validate(input: CreateInput) {
    ...
    if (input.status === "ACTIVE" && !hasListingPhoto(input.payload)) {
...
class UpdateListingWorkflow extends ListingWriteWorkflow<UpdateInput, Awaited<ReturnType<typeof listingRepository.update>>> {
  ...
  protected async authorize(input: UpdateInput) {
    ...
    if (!listing || !canManageListing(input.userId, listing)) {
      throw new DomainError("Listing not found.", "LISTING_NOT_FOUND");
    }
    if (listing.status === "ACTIVE" && !hasListingPhoto(input.payload)) {
      throw new DomainError("Active listings require at least one photo.", "LISTING_PHOTO_REQUIRED");
    }
  }
...
export const listingService = {
  ...
  async updateStatus(userId: string, listingId: string, status: string) {
    ...
    if (status === "ACTIVE" && !hasListingPhoto(listing)) {
      throw new DomainError("Upload at least one photo before publishing.", "LISTING_PHOTO_REQUIRED");
    }
