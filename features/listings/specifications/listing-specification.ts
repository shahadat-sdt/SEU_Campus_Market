...
export function isValidListingPayload(payload: ListingPayload) {
  return (
    ...
    (!payload.imageUrl || isHttpsUrl(payload.imageUrl)) &&
    Number.isFinite(payload.price) &&
    payload.price > 0
  );
}

export function hasListingPhoto(input: Pick<ListingPayload, "imageUrl"> & { imageUrls?: string[] }) {
  return Boolean(input.imageUrls?.some(isHttpsUrl) || isHttpsUrl(input.imageUrl));
}
