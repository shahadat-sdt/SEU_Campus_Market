...
import { listingCategories, listingConditions } from "@/shared/lib/constants";
...
export function ListingForm({ action, initial, listingId }: ListingFormProps) {
  ...
  const activeListingNeedsPhoto = initial?.status === "ACTIVE" && !images.length;
  ...
  function submit(intent: "draft" | "publish") {
    ...
    if (!form) return;
    if (intent === "publish" && !images.length) {
      setUploadError("Upload at least one item photo before publishing.");
      return;
    }
    ...
    <div className="grid gap-5">
      <form ref={formRef} action={action} className="grid gap-5">
        ...
        <section className="rounded-md border bg-card p-4">
          ...
          <p className="mt-1 text-sm text-muted-foreground">At least one uploaded item photo is required before the listing can go live.</p>
          <label
        ...
        <div className="flex flex-wrap gap-3">
          <Button
            type="button"
            variant="outline"
            disabled={isPending || isUploading || activeListingNeedsPhoto}
            onClick={() => submit("draft")}
          >
            {isPending && <Loader2 className="h-4 w-4 animate-spin" />} {initial?.status === "ACTIVE" ? "Save changes" : "Save draft"}
          </Button>
        ...
        <section className="rounded-md border bg-card p-4 shadow-campus">
          <div className="grid gap-4 sm:grid-cols-[160px_1fr]">
            ...
            <div className="space-y-2">
              ...
              <Button disabled={isPending || isUploading || !images.length} onClick={() => submit("publish")}>
