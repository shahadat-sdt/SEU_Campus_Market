...
import { deleteListing, updateListingStatus, updateOrder } from "@/features/marketplace/actions";
import { listingStatuses, orderStatuses } from "@/shared/lib/constants";
import { safeImageUrl } from "@/shared/lib/image";
import type { SellerDashboardData } from "@/features/marketplace/api/marketplace-api";
...
export function SellerDashboard({ data }: { data: SellerDashboardData }) {
  const [listings, sales, pendingOrders] = data;
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-8">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <MetricCard icon={TrendingUp} title="Total sales" value={money(revenue)} />
        <MetricCard icon={ShoppingBag} title="Open orders" value={pendingOrders.length.toString()} />
        <MetricCard icon={PackageCheck} title="Active listings" value={activeCount.toString()} />
      </div>
