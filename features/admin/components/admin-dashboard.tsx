...
import type { AdminDashboardData } from "@/features/marketplace/api/marketplace-api";
...
export function AdminDashboard({ admin, roleError, listingError, data }: AdminDashboardProps) {
  const [
    reports,
    users,
    listings,
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-8">
      ...
      {listingError === "photo" && (
        <div className="mb-6 rounded-md border border-amber-200 bg-amber-50 p-3 text-sm text-amber-950">
          A listing needs at least one uploaded photo before it can be marked active.
        </div>
      )}
      ...
      <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
        <MetricCard icon={AlertTriangle} title="Open reports" value={openReports.toString()} />
        <MetricCard icon={CircleDot} title="Active listings" value={activeListings.toString()} />
        <MetricCard icon={ShieldCheck} title="Hidden listings" value={hiddenListings.toString()} />
        <MetricCard icon={Star} title="Sponsored" value={sponsoredListings.toString()} />
        <MetricCard icon={UsersRound} title="Orders" value={orderCount.toString()} />
      </section>
