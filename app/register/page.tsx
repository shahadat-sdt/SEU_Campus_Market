...
import { register } from "@/features/marketplace/actions";
    ...
    <main className="mx-auto grid min-h-[calc(100vh-66px)] max-w-md place-items-center px-4 py-10">
      <Card className="w-full shadow-campus">
        <CardHeader>
          ...
          <CardDescription>Use your SEU email to join the campus marketplace.</CardDescription>
        </CardHeader>
        <CardContent>
          {error && (
            <p className="mb-4 rounded-md border border-destructive/30 bg-destructive/10 p-3 text-sm text-destructive">
              {error === "email"
                ? "Please use an email ending in @seu.edu.bd."
                : error === "exists"
